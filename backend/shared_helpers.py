...
VIDEO_DIRS = {
    "tool1": "voiceover/audio",
    "tool2": "videos_static/temp",
    "tool3": "videos_animated/temp"
}
...
def _get_plan_limit_from_db(plan, tool):
    """Fetch per-tool limit for a plan from the plans table."""
    conn = get_conn()
    c    = conn.cursor()
    c.execute("SELECT tool1_videos, tool2_videos, tool3_videos FROM plans WHERE name=?", (plan,))
    row  = c.fetchone()
    conn.close()

    if row:
        idx = int(tool.replace("tool", "")) - 1
        return [row["tool1_videos"], row["tool2_videos"], row["tool3_videos"]][idx]
    return PLAN_LIMITS.get(plan, {}).get(tool, 1)


def add_video_for_user(user_id, tool, video_id, filename):
    """
    Adds a new video/audio record for a user.
    ...
    if not c.fetchone():
        c.execute(
            "INSERT INTO users (id, plan, role, created_at) VALUES (?, 'free', 'user', ?)",
            (user_id, now)
        )
        conn.commit()
    ...
    try:
        c.execute(
            "INSERT INTO user_videos (user_id, video_id, file_path, tool, created_at) VALUES (?, ?, ?, ?, ?)",
            (user_id, video_id, filename, tool, now)
        )
    except sqlite3.IntegrityError:
        c.execute(
            "UPDATE user_videos SET file_path=?, created_at=? WHERE video_id=?",
...
def get_user_videos(user_id):
    ...
    Returns:
        List of dicts with keys: video_id, audio_id, filename, tool, created_at
    """
    conn = get_conn()
    c    = conn.cursor()
    c.execute(
        "SELECT video_id, file_path, tool, created_at FROM user_videos WHERE user_id=? ORDER BY created_at DESC",
        (user_id,)
    )
    rows = c.fetchall()
    conn.close()
...
def delete_video(user_id, video_id):
    ...
    c    = conn.cursor()
    c.execute(
        "SELECT tool, file_path FROM user_videos WHERE user_id=? AND video_id=?",
        (user_id, video_id)
    )
    row = c.fetchone()
    ...
    file_path = os.path.join(VIDEO_DIRS[row["tool"]], user_id, row["file_path"])
...
def delete_old_videos(user_id, tool, max_allowed=None):
    ...
    if max_allowed is None:
        ...
        max_allowed = _get_plan_limit_from_db(plan, tool)
...
def check_plan_limit_legacy(user_id, tool):
    ...
    plan        = row["plan"]
    used        = sum(1 for v in get_user_videos(user_id) if v["tool"] == tool)
